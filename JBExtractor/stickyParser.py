...
def plum(db):
    ...
    for r in range(len(db_df)):
        textList = []
        dataList = []
        if db_df.iloc[r]['Text'] != "":
            texts = db_df.iloc[r]['Text'].split('\n')
...
if __name__ == "__main__":    
    ...
    if flag > 0:
        ...
        file = open(plumFile + 'stickynotemetadata-' + now + '.txt','w')
        ftype = "a"
        pages = "all"
        filesize = os.path.getsize(plumFile)
        freeleaf = [ ]
        freetrunk = [ ]
        btreepages = [ ]
        leafpages = [ ]
